...
const CityWeatherCard = ({ city }: CityWeatherCardProps) => {
  ...
  return (
    <section className="w-11/12 min-h-full border-0 shadow rounded-md p-3 flex flex-col items-center mt-3 mb-4 bg-black/60 max-w-md">
      <WeekDayMenu
        today={city.forecastEvery3HoursCollection[0].weekday}
        selectedDay={selectedDay}
        setSelectedDay={setSelectedDay}
      />
      <div className="w-60 h-60 my-10 bg-none rounded-full"></div>
      <div className="pl-12 w-full ">
        ...
        <ul className="flex flex-col text-secondaryText my-5">
          <li className="mb-2">
            ...
            <span className="mx-2 font-bold text-white">{humidity}% </span>
            Humidity
          </li>
          <li>
            <FiWind className="inline" />
            <span className="mx-2 font-bold text-white">{speed}km/h </span> Wind
          </li>
        </ul>
      </div>
      <Link
        to={`/details/${city.name}`}
        className="w-[calc(100%+24px)] flex justify-center text-white bg-accent h-16 translate-y-3 rounded-b-md"
      >
        <div className="bg-accent h-full flex items-center font-medium text-black">
          Details
